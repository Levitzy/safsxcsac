...
const fetchTemporaryEmail = async (statusMsg) => {
    ...
    try {
        ...
        if (response.data && response.data.email_address && response.data.api_session_id) {
            await statusMsg.edit({ content: `📬 Temporary email received: \`${response.data.email_address}\`` });
            return { email: response.data.email_address, sessionId: response.data.api_session_id };
        } else {
            throw new Error('Invalid response from temporary email API.');
        }
    } catch (error) {
