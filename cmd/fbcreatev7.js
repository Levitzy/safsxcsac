...
const STRICT_ACCEPT_LANGUAGE = 'en-US,en;q=0.9';

const FALLBACK_MOBILE_USER_AGENT = 'Mozilla/5.0 (Linux; Android 12; Pixel 6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/121.0.0.0 Mobile Safari/537.36';
...
const generateUserAgentObject = () => {
    try {
        ...
        if (!userAgentInstance || !userAgentInstance.data || !userAgentInstance.toString()) {
            console.warn('[fbcreatev7] UserAgent library returned invalid instance, using fallback.');
            return { data: { mobile: true, platform: 'Android', deviceCategory: 'mobile', brands: [{brand: "Not_A Brand", version: "99"}, {brand: "Chromium", version: "121"}, {brand: "Google Chrome", version: "121"}] }, toString: () => FALLBACK_MOBILE_USER_AGENT };
        }
        if (!userAgentInstance.data.brands || userAgentInstance.data.brands.length === 0) {
             userAgentInstance.data.brands = [{brand: "Not_A Brand", version: "99"}, {brand: "Chromium", version: userAgentInstance.data.version || "121"}, {brand: "Google Chrome", version: userAgentInstance.data.version || "121"}];
        }
        if (!userAgentInstance.data.platform) {
            userAgentInstance.data.platform = "Android";
        }
        userAgentInstance.data.mobile = true;
        return userAgentInstance;
    } catch (error) {
        console.warn(`[fbcreatev7] Error generating user agent: ${error.message}. Using fallback.`);
        return { data: { mobile: true, platform: 'Android', deviceCategory: 'mobile', brands: [{brand: "Not_A Brand", version: "99"}, {brand: "Chromium", version: "121"}, {brand: "Google Chrome", version: "121"}] }, toString: () => FALLBACK_MOBILE_USER_AGENT };
    }
};

const humanLikeDelay = (minMilliseconds = 1600, maxMilliseconds = 4200) => {
    const delay = Math.random() * (maxMilliseconds - minMilliseconds) + minMilliseconds;
    return new Promise(resolve => setTimeout(resolve, delay));
};

const shortHumanLikeDelay = (minMilliseconds = 700, maxMilliseconds = 1900) => {
    const delay = Math.random() * (maxMilliseconds - minMilliseconds) + minMilliseconds;
    return new Promise(resolve => setTimeout(resolve, delay));
};
...
const fetchOtpFromTempEmail = async (tempEmailSessionId, statusMsg, emailAddress) => {
    ...
    while (Date.now() - startTime < OTP_POLL_DURATION_MS) {
        ...
        if (currentTime - lastPollTime >= (OTP_POLL_INTERVAL_SECONDS * 1000)) {
            ...
            } catch (error) { console.warn(`[fbcreatev7] Error polling OTP: ${error.message}`); }
...
const extractFormDataV2 = (html, formSelector = 'form[action*="/reg/"], form[id="registration_form"]') => {
    ...
    $('script').each((_, scriptTag) => {
        ...
        } catch(e) { console.warn("[fbcreatev7] Minor error parsing JSON from script tag.");}
    ...
    if (!formData.fb_dtsg) {
        ...
        console.warn("[fbcreatev7] fb_dtsg not found, generated fallback.");
    }
    if (!formData.jazoest) {
        let sum = 0; for (let i = 0; i < (formData.fb_dtsg || '').length; i++) sum += (formData.fb_dtsg || '').charCodeAt(i); formData.jazoest = '2' + sum;
        console.warn("[fbcreatev7] jazoest not found, generated fallback.");
    }
    if (typeof formData.lsd === 'undefined' || !formData.lsd) {
        ...
        console.warn("[fbcreatev7] lsd not found, generated fallback.");
    }
...
const attemptRegistrationSubmission = async (session, payload, refererUrl, statusMsg, proxyInUse) => {
    ...
    for (const endpoint of submitEndpoints) {
        ...
        try {
            ...
            if (responseText.toLowerCase().includes("confirm you're human") || responseText.toLowerCase().includes("confirm that you are not a robot") || finalUrl.includes("/challenge/") || finalUrl.includes("/checkpoint/challenge/") || responseText.toLowerCase().includes("solve this security check")) {
...
const extractUidAndProfile = async (cookieJar, responseText, finalUrl) => {
    ...
    else { const xsMatch = cookieString.match(/xs=([^;]+)/); if (xsMatch && xsMatch[1]) { try { const xsDecoded = decodeURIComponent(xsMatch[1]); const xsParts = xsDecoded.split('%3A'); if (xsParts.length > 1 && /^\d{10,}$/.test(xsParts[0]) && xsParts[0] !== '0') uid = xsParts[0]; } catch (e) { console.warn("[fbcreatev7] Error decoding xs cookie for UID."); } } }
...
const sendCredentialsMessage = async (channel, email, password, uid, profileUrl, accountName, tempEmailProviderName, outcome, proxyUsed, accountNum, totalAccounts, tempEmailDataForButton = null, originalInteractionUser) => {
    ...
    try {
        ...
        return sentMessage;
    } catch (sendError) { try { await channel.send({ content: `Error sending embed for Account ${accountNum}, fallback: ${outcome.title} - Email: ${email} Pass: ${password} UID: ${uid}`}); } catch (fallbackError) {console.error("[fbcreatev7] Fallback send error", fallbackError)} return null; }
};

async function createSingleFacebookAccount(channel, originalInteractionUser, effectiveProxyString, userAgentInstance, accountNum, totalAccounts, providerForEmail) {
    const genPassword = fakePassword(); const genName = fakeName();
    let statusMsg; let tempEmailData = null; let sessionForProxyCheck;
    let tempEmailProviderActual = 'N/A'; const userAgentString = userAgentInstance.toString();
    ...
    try {
        ...
        const session = createAxiosSession(userAgentInstance, effectiveProxyString); sessionForProxyCheck = session;
        ...
        if (statusMsg && statusMsg.deletable) await statusMsg.delete().catch(e => { console.warn(`[fbcreatev7] Could not delete status message: ${e.message}`); });
        return outcome;
    } catch (error) {
        ...
        if (statusMsg && statusMsg.deletable) await statusMsg.delete().catch(e => { console.warn(`[fbcreatev7] Could not delete status message after error: ${e.message}`); });
        throw error;
    }
}
...
module.exports = {
    name: 'fbcreatev7',
    description: 'Concurrently creates Facebook account(s) with anti-detection measures. Usage: !fbcreatev7 [provider] [count] [proxy] OR !fbcreatev7 [count] [proxy]. Provider (default: random), count (default: 1), proxy are optional.',
    admin_only: false,
    async execute(message, args) {
        ...
        try {
            ...
            const initialReplyText = `🚀 Starting concurrent creation process for ${amountAccounts} Facebook account(s)${providerInfo}. This may take some time...`;
            
            try {
                initialReplyMessage = await message.reply(initialReplyText);
            } catch (e) {
                console.error("[fbcreatev7] Failed to send initial reply, sending to channel instead:", e.message);
            ...
            for (let i = 1; i <= amountAccounts; i++) {
                ...
                accountCreationPromises.push(
                    createSingleFacebookAccount(message.channel, message.author, proxyString, userAgentInstanceForThisAccount, i, amountAccounts, providerName)
                );
            }
                    ...
                    if (outcome && typeof outcome.type === 'string' && outcome.type === "success_otp_fetched") {
                        ...
                        failureCount++;
                         if (!outcome || typeof outcome.type !== 'string') {
                            console.warn('[fbcreatev7] Fulfilled promise did not return a standard outcome object:', result.value);
                        }
                    }
                } else {
                    failureCount++;
                    console.error(`[fbcreatev7] A task for account creation was rejected. Reason:`, result.reason && result.reason.message ? result.reason.message : result.reason);
                }
            });
            ...
            if (finalMessageChannel && typeof finalMessageChannel.send === 'function') {
                await finalMessageChannel.send(`🏁 Concurrent batch creation process finished. Attempts: ${amountAccounts}, OTP Verified Successes: ${successCount}, Checkpoints/Failures: ${failureCount}. Check individual messages for details.`);
            } else {
                console.error("[fbcreatev7] Could not determine a valid channel to send the final summary message.");
            }

        } catch (error) {
            console.error('CRITICAL ERROR in fbcreatev7 execute:', error.message, error.stack);
            try {
                if (message && message.channel && typeof message.channel.send === 'function') {
                    await message.channel.send(`🚨 An unexpected critical error occurred with the command. Details: ${error.message}. Dev has been notified (check console logs).`).catch(e_final => {
                        console.error("[fbcreatev7] Failed to send critical error message to Discord channel:", e_final.message, e_final.stack);
                    });
                } else {
                     console.error("[fbcreatev7] CRITICAL: message or message.channel was undefined in top-level catch block. Error:", error.message);
                }
            } catch (e_panic) {
                console.error("[fbcreatev7] PANIC: Could not even attempt to send error message to channel in top-level catch.", e_panic.message, e_panic.stack);
            }
        }
    }
};
